...
def process_all(raw_data_dir, variable_name_list, save_dir, args):
    ...
    variable_date_dict = {variable: {datafile.name[:21]: datafile for datafile in os.scandir(raw_data_dir) if f"{variable}lt1-24.npy" in datafile.name} for variable in variable_name_list}
    n_days = len(variable_date_dict["rr"])
    for variable, date_dict in variable_date_dict.items():
        if len(date_dict) != n_days:
            raise ValueError(f"Day number for variable {variable} is different than for variable rr")
    ...
    for day_count, date in enumerate(variable_date_dict["rr"]):
        ...
        if args.verbose >= 1:
            print(f"Day {date[:-1]}")
        array_at_date_set = [np.load(variable_date_dict[variable][date].path) for variable in variable_name_list]
        if args.verbose >= 2:
            print(f"Merging...")
        merged_array = np.array([array for array in array_at_date_set]) # [nbr_var, x, y, leadtimes, members]
        if args.verbose >= 2:
            print(f"Splitting{' and cropping' * args.crop}...")
        splitted_by_members_arrays = np.array([merged_array[:, :, :, :, member] for member in range(16)]) # [members, nbr_var, x, y, leadtimes]
        for member, splitted_by_members_array in enumerate(splitted_by_members_arrays): # [nbr_var, x, y, leadtimes]
            splitted_by_leadtime_arrays = np.array([splitted_by_members_array[:, :, :, leadtime] for leadtime in range(24)]) # [leadtimes, nbr_var, x, y]
            for leadtime, splitted_by_leadtime_array in enumerate(splitted_by_leadtime_arrays): # [nbr_var, x, y]
                ...
                with open(f"{save_dir_splitted}labels.csv", "a", encoding="utf8") as file:
...
def handle_patch(dataframe, data_dir, save_dir, max_files_loaded, args):
    ...
    for patch in range(1, n_patch + 1):
        if args.verbose >= 2: print(f"Patch {patch}/{n_patch}")
...
def load(dataframe, data_dir, save_dir, beg, end, patch, args):
    ...
    """
    n_tot = end - beg
    if args.verbose >= 3: print(f"Loading data, {n_tot} files...")
    data = []
    start_time = perf_counter()
    for index in range(beg, end):
        ...
        with open(f"{save_dir}labels.csv", "a", encoding="utf8") as file:
            file.write(f"{row['Name']},{row['Date']},{row['Leadtime']},{row['Member']},{patch},{index-beg}\n")
    return data
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--save_path", type=str, default="/cnrm/recyf/NO_SAVE/Data/users/gandonb/data_for_importance_sampling/", help="Path where data is saved")
